      ...
      <Box sx={{...pricingcardstyle.rootSx }}>
       ...
       <Box sx={{...pricingcardstyle.totalPricingBoxSx}}>
                    ...
                    return(
                      <Box key={index} sx={{...pricingcardstyle.totalBodySx}}>
                           <Card sx={{...pricingcardstyle.cardSx}}>
                             ...
                             { val?.pricingList && val?.pricingList?.map((val,index)=>{
                              return(
                                <Box key={index} sx={{...pricingcardstyle.pricingListSx}}>
                                <Box sx={{...pricingcardstyle.totalList}}>
                                  <IconButton sx={{padding:'0px'}} disableRipple>
                                      {val?.icon}
                                  </IconButton>
                               <Typography sx={{...pricingcardstyle.pricingListRow,textDecoration: val?.isCancel ?  'line-through' : 'none'}}>
                                {val?.listPoints}
                               </Typography>
                                </Box>
                                 </Box>
                             ...
                             <Box>
                                 <Typography sx={{...pricingcardstyle.pricingHeadSx,pt:'27px'}}>
                                   {val?.subcriptionTitle}{val?.subcriptionTitle=== 'Free' ? '' : <>
                                   /<Box component={'span'}sx={{...pricingcardstyle.subscriptionDueSx}}>{val?.subscriptionDue}</Box>
                                   </>}
                                  </Typography>
                             <CardActions sx={{justifyContent:'center'}}>
                                   <Button sx={{...pricingcardstyle.getBtnSx}} onClick={()=>onGetStartFunc()}>{val?.getStartedbtn}</Button>
                             </CardActions>
                             </Box>
         ...
         <Box sx={{...pricingcardstyle.totalPricingBoxvarSx}}>
                <Box>
                {pricingCardVariationTwo && pricingCardVariationTwo?.map((val,index)=>{
                    return(
                      <Box key={index}>
                        <Box  sx={{...pricingcardstyle.totalBodySx}}>
                           <Card sx={{...pricingcardstyle.cardSx,pt:variation2 ? '18px': '24px',border:active===index ? '2px solid #665CD7':'none'}} onClick={()=>onPricingChange(val,index)}>
                            {active ===index  ?<Box sx={{...pricingcardstyle.selectIconSx}}>
                             ...
                             <CardContent sx={{p:'0px'}}>
                              <Box>
                                 <Typography sx={{...pricingcardstyle.pricingHeadVarSx}}>
                                   {val?.title}
                                  </Typography>
                                  <Typography sx={{...pricingcardstyle.pricingSubVarSx}}>
                                    {val?.headerName}
                                  </Typography>
                              </Box>
                              <Box>
                                 <Typography sx={{...pricingcardstyle.pricingsubSx,pt:'24px',pb:'20px'}}>
                                   {val?.subcriptionTitle}{val?.subcriptionTitle=== 'Free' ? '' : <>
                                   /<Box component={'span'}sx={{...pricingcardstyle.subscriptionDueSx}}>{val?.subscriptionDue}</Box>
                                   </>}
                                  </Typography>
                             ...
                             { val?.pricingList && val?.pricingList?.map((val,index)=>{
                              return(
                                <Box key={index} sx={{...pricingcardstyle.pricingListSubSx}}>
                                <Box sx={{...pricingcardstyle.totalList}}>
                                  <IconButton sx={{padding:'0px'}} disableRipple>
                                      {val?.icon}
                                  </IconButton>
                             ...
                             <CardActions sx={{justifyContent:'center',pt:'24px !important',p:0}}>
                                   <Button sx={{...pricingcardstyle.getBtnSx,maxWidth: variation2 ?  'inherit': '204px'}} onClick={()=>onGetStartFunc()}>{val?.getStartedbtn}</Button>
                             </CardActions>
