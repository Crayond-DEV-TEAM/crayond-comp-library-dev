...
const Column: React.FC<Props> = ({ column, columnIndex }) => {
  ...
  return (
    <>
      <Container style={{ ...columnContainerRootStyle }}>
        <TitleContainer style={{ ...columnTitleBoxStyle }}>
          <Title style={{ ...columnTitleStyle }}>{column.title}</Title>
        </TitleContainer>
            ...
            <RowContainer ref={prov.innerRef} {...prov.droppableProps}>
              {column.tasks.map((task, taskIndex) => (
                <Row
                  ...
                  task={task}
                  index={taskIndex}
                  isDraggingStart={snapshot.isDraggingOver}
                />
              ))}
              {prov.placeholder}
              <DropshadowContainer>
        ...
        >
          ...
          <span style={{ color: '#665CD7', ...addButtonLabelStyle }}>
            {' '}
